...
public static class GizmosUtils
{
    ...
    public static void DrawVector(Vector3 pos, Vector3 direction, float thickness = defaultThickness, bool drawArrowCap = true)
    {
        ...
        if (drawArrowCap)
        {
            Vector3 right = Quaternion.LookRotation(direction) * Quaternion.Euler(arrowHeadAngle, 180 + arrowHeadAngle, 0) * new Vector3(0, 0, 1);
            Vector3 left = Quaternion.LookRotation(direction) * Quaternion.Euler(-arrowHeadAngle, 180 - arrowHeadAngle, 0) * new Vector3(0, 0, 1);
